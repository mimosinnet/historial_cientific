% Header {{{
\documentclass[english,a4paper,12pt,titlepage]{report}
\usepackage[T1]{fontenc}

\usepackage{multicol}
\usepackage{graphicx}
% url
\usepackage[spaces,hyphens]{url}
\usepackage[colorlinks,allcolors=afin]{hyperref}
\renewcommand{\UrlFont}{\footnotesize}
% fancyhdr
\usepackage{fancyhdr}
\pagestyle{fancy}
\addtolength{\headwidth}{3cm}
\fancyhf{}
\lhead{\includegraphics[width=2cm]{AFIN-Barcelona.png}}
\fancyhead[R]{\leftmark \\ \rightmark}
\fancyfoot[C]{\thepage}

%
\usepackage{xcolor}
\definecolor{afin}{RGB}{112, 76, 116}

% Geometry https://texdoc.org/serve/geometry.pdf/0
% \usepackage[left = 2.5cm, right = 2cm, marginparwidth=0cm, includeheadfoot]{geometry}
\usepackage[left=25mm,right=20mm,top=20mm,bottom=20mm, headsep=10mm]{geometry}

% \renewcommand{\cleardoublepage}{}
% \renewcommand{\clearpage}{}

\usepackage{sectsty}
\sectionfont{\color{afin}}

\renewcommand{\chaptername}{Section}

% }}}

\begin{document}

	\begin{titlepage}
		\centering

	\hspace{0pt}	
	\vspace*{\fill}

	    \Huge\textcolor{afin}{\bf AFIN Scientific Production}
	    \vspace{2cm}
	    
	    \large{\bf 2020-2025}
	    
	    \vspace{2cm}
	    
	    \includegraphics[width=5cm]{AFIN-Barcelona.png}

	    \vspace{3cm}

		\begin{tabular}{c}
			\textcolor{afin}{\textbf{AFIN RESEARCH GROUP}} \\
			Edifici B13, c/ Vila Puig s/n \\
			Universitat Autònoma de Barcelona \\
			08193 Bellaterra (Cerdanyola del Vallès) \\
			Tel.:+34 935 814 640 \\
			gr.afin@uab.cat \\
			\textcolor{afin}{\large\textbf{http://afin-barcelona.uab.cat}}
		\end{tabular}
	
	\vspace{\fill}
	
		\end{titlepage}
		
\tableofcontents

  % Presentation {{{
	\chapter{Presentation}

	AFIN is an interdisciplinary, inter-university and international research group based at the Universitat Autònoma de Barcelona (UAB). It focuses on fundamental and applied research in the fields of social sciences, humanities, and health sciences. The group's mission is to explore the complexities of human reproduction and the multifaceted nature of inequality, examining how factors such as age, sex, gender, disability, race, ethnicity, religion, embodiment, nationality, and their intersections affect individuals and societies. Committed to disseminating the results of its research to the public and diverse communities, AFIN brings together experts from diverse disciplinary fields: anthropology (social, cultural and medical), sociology, education, law, medicine, psychology, nursing, and social work.\\\
	In 2015, the group inaugurated the AFIN Centre, a pioneering outreach centre endorsed by the Universitat Autònoma de Barcelona, with the mission to transform research insights into community services. By 2021, Inari Global Social Action- AFIN was established as a legal entity dedicated to providing expert guidance, support, specialized training, and counselling in the realm of human reproduction. This encompasses a broad spectrum of issues related to family dynamics, childhood development, gender identity, sexual and reproductive health, the reproductive cycle, bodily health, and disease. The comprehensive range of services provided by AFIN-UAB is grounded in extensive research spanning social sciences, health, and humanities. The development of applied research and has provided the AFIN-UAB Research Group with specialized expertise to effectively guide, support, train, and counsel both institutions and individuals navigating the complexities of human reproduction.\\\
	

  % }}}
	
  % Research Lines {{{
	\chapter{Research Lines}
	\begin{itemize}
		\item The Dynamics of Parenting Roles.
		\item Ethical Considerations in Third-Party Reproduction.
		\item Global Perspectives on Cross-Border Reproductive Care (CBRC).
		\item Menstrual Health and Disorders.
		\item Privacy in Reproductive Health.
		\item Diversity in Childhood and Youth Development. 
		\item Sexual and Reproductive Health and Education Programs.
		\item Parenting Strategies for Current Family Structures.
		\item Gender Roles in Care-giving.
		\item Parenthood and Disability.  
	\end{itemize}
	
	\chapter{Group Awards and Acknowledgements}
	\begin{itemize}
		\item September 14, 2021 AFIN research group was awarded the First Prize of the Annual Apply Awards of the European Association of Social Anthropologists (EASA) Applied Anthropology Network for the AFIN-ART project, a service of accompaniment in decisions and reproductive processes developed from Social Anthropology. Alexandra Desy, Master and PhD student at UAB, and Bruna Alvarez, PhD and senior lecturer at UAB, both members of AFIN and the AFIN-ART project, received the award on behalf of the AFIN group, in Prague.
		\item Diana Marre received the ICREA Acadèmia award. This award is granted by the Catalan Institution for Research and Advanced Studies in recognition of the research work by professors in Catalan public universities and, at the same time, the award contributes to retaining the research talent within the Catalan university system. The award was given to 30 researchers. Marre has been awarded again in the category of humanities for her work on reproductive politics and practices in Spain. With the recognition of ICREA Academia, the 30 professors awarded will receive 40,000 euros annually each, for a period of five years (2020-2024). These distinctions are aimed at university teaching staff who are doing research and teaching, and especially, those who are in the fully active phase and the expansion of their research activity.
		\item In 2017, the Agency for Management of University and Research Grants (AGAUR) acknowledged AFIN as Consolidated Research Group, a categorization that was renewed and granted funding in 2021.
	\end{itemize}
  % }}}

  % Current Members {{{
	\chapter{Current Members}
	\begin{center}
	\textbf{Principal Investigator}: Diana Marre (UAB)\\
	
	\begin{multicols}{2}

\textbf{Researchers} \\
\vspace{0.2cm}
 Anna Molas (UAB) \\
 Aranzazu Gallego Molinero (UGR) \\
 Beatriz San Román (UAB) \\
 Bruna Alvarez (UAB) \\
 Carolina Remorini (UAB)\\
 Chandra Kala Clemente Martínez (UAB) \\
 Estel Malgosa (UAB) \\
 Irene Salvo Agoglia (UAB)\\
 Joan Pujol-Tarrés (UAB) \\
 Laura Sanmiquel (UAB) \\
 Lynne McIntyre (UAB) \\
 Lucas Platero (URJC) \\
 Maria Romeu-Labayen (UAB-UB) \\
 Marisela Montenegro-Martínez (UAB) \\
 María José Rodríguez (UA) \\
 Paola Galbany-Estragués (UB) \\
 Sofia Gaggiotti-Marre (H. Sant Pau-UAB) \\
 Valle Medina (UGR) \\

\vfill\null
\columnbreak

\textbf{Associate researchers} \\
\vspace{0.2cm}

Alexandra Desy (UAB) \\
Ana Cerezuela González (UAB)\\
Giulia Colavolpe Severi (UAB)\\
Laura Sanmiquel (UAB) \\
Mariana Campos Lichstejn (UAB)\\
Paula Martone (UAB)\\
Vanessa Mantilla (UAB-FPU)\\

\vspace{0.3cm}

	\textbf{Transnational network} \\
\vspace{0.2cm}	
Anna Uziel (UFRJ, Brazil) \\
Carla Villalta (UBA-Conicet, Argentina) \\
Claudia Fonseca (UNRGdS–Brazil) \\
Fernanda Cruz Rifiotis (UNRGdS–Brazil) \\
Hugo Gaggiotti (UWE, UK) \\
Jessaca Leinaweaver (Brown U., USA) \\
Lucía Ariza (UBA-Conicet, Argentina)\\
Marcin Smietana (U. of Cambridge, UK) \\
Miguel Gaggiotti (Bristol U., UK) \\
Salome Lima (UBA-Conicet, Argentina) \\
Soledad Gesteira (UBA-Conicet, Argentina)\\
Theresa Trimmel (U. of Southampton, UK) \\

	
	\end{multicols}
\end{center}
  % }}}


  \chapter{Scientific Production}

  \section{Articles published in indexed journals}
  
  Included in the list there are Special Issues produced by AFIN: \textbf{1)} 2022: De Zordo, Silvia; Marre, Diana; Smeietana, Marcin. Double Special Issue: Demographic Anxieties in the Age of ‘Fertility Decline’, \textit{Medical Anthropology}, 41(6-7). \textbf{2)} 2021: Gallego Molinero, Aranzazu; Clemente-Martínez, Chandra Kala, Irregularities in transnational adoptions and child appropriations: Challenges for reparation practices. \textit{Childhood}, 28(4).
  
  	\input{include/UWPY76KY.tex}
  \section{Articles in Scientific Journals}
  	\input{include/442HYH2P.tex}
  \section{Books}
  	\input{include/2QEDV4YS.tex}
  \section{Book Chapters}
  	\input{include/YPAJSHFV.tex}
  \section{Conference Presentations}
  	\input{include/R42QPUFI.tex}
  
  \chapter{Projects}
  \section{Current competitive projects}
    \input{include/36U9M8UA.tex}
  \section{Former competitive projects}
    \input{include/N7K6GRL2.tex}
  \section{Competitive Transference Grants}
     \input{include/BRL62EC2.tex} 

  \section{Non-competitive Projects / Agreements}
    \input{include/PWH78LJ5.tex}

  \chapter{Internationalisation}

  \section{Organisation of International Conferences and Workshops}
    \input{include/IARNE42E.tex}
  \section{Participation in International Networks}
    \input{include/FYCMV3HK.tex}
  \section{International stages}
     \input{include/Q5E6D3QA.tex}  
     
  \chapter{Research Training}

  \section{Completed doctoral theses}
	\input{include/H9FYFWQY.tex}
  \section{Doctoral thesis in progress}
  	\input{include/YUGQZ453.tex}
  \section{Final Master Theses}
  	\input{include/PJNPKBEK.tex}
  \section{Post-Doctoral Grants}
  	\input{include/3HN4HS2I.tex}
  \section{Pre-Doctoral Grants}
  	\input{include/TZFYNY2Y.tex}

  \chapter{Knowledge Transfer}

  \section{AFIN free monthly publication}
  	\input{include/BW926N6F.tex}
  \section{Workshops}
  	\input{include/364GBY9U.tex}
  \section{Podcasts}
  	\input{include/QR2SVS4L.tex}
  \section{TV Presentations}
  	\input{include/H4IXXNVT.tex}
   \section{Maganize Articles}
   	\input{include/P5RR9R5K.tex}
   \section{Blog Posts} 
   	\input{include/ARXRRBMK.tex}

\end{document}
